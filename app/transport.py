# =============================================================================
# transport.py - V2.2.6 Production Final (Edge Node Hardened)
# 模組名稱：數據傳輸層 (RS485/TCP)
# 修正亮點：
#   - [Fix] 移除防禦過當的 JK 尾部特徵檢查，解除 0x01 靜態幀被誤殺的問題 (V2.2.6)
#   - [Fix] 統一鎖定 JK 物理邊界為精準的 300 bytes，消滅越界吞噬 Modbus 封包的問題
# =============================================================================
...
class BaseTransport(ABC):
    ...
    def _is_valid_jk_frame(self, frame: bytes, expected_len: int) -> bool:
        """ 
        🟢 JK 原生封包嚴格校驗防線 
        """
        ...
        if not frame.startswith(HEADER_JK):
            return False
            
        # 🚨 [V2.2.6 修復] 移除尾部特徵檢查！
        # 因為 0x01 靜態幀的倒數第2個 Byte 會跳動，強制檢查會導致誤殺真實封包。
        # 只要長度精準且表頭正確，就交給 decoder 去做物理邊界 (LIMITS) 攔截。
        return True
    ...
    def _extract_packets(self, buffer: bytearray) -> Generator[Tuple[int, bytes], None, None]:
        while True:
            ...
            if is_jk_first:
                ...
                # 🟢 統一使用精準長度 300 bytes 避免吞噬 Modbus 封包
                p_len = 300 
                
                if len(buffer) >= jk_idx + p_len:
                    frame = bytes(buffer[jk_idx : jk_idx + p_len])
                    
                    if self._is_valid_jk_frame(frame, p_len):
                        yield p_type, frame
                        del buffer[:jk_idx + p_len] # 安全切除
                        continue
                    else:
                        if self.debug_raw_log:
                ...
                if cmd_len == -1:
                    ...
                    del buffer[:mb_idx + cmd_len] 
                    continue
                else:
                    # 遇到假 Header，只推進 2 bytes
                    del buffer[:mb_idx + 2]
                    continue
